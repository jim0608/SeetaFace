...
import com.seeta.sdk.seetafaceEngine.helper.FeaturePointFaceHelper
 ...
 * @Describe:
 */
class FeaturePointActivity : AppCompatActivity() {
    ...
    private var helper: FeaturePointFaceHelper? = null
    ...
    private fun detection() {
        val bitmap = helper?.detection()
        if (bitmap != null) {
            Glide.with(this).load(bitmap).into(dataBinding.imageView)
            showBit(bitmap)
        }
    }
